...
async def main_page():
    with db.engine.connect() as con:
        data = pd.read_sql_query(
            select(AllBrands.name, AllBrands.short_name).join(BrandsData, AllBrands.short_name == BrandsData.brand).distinct(),
            con).to_dict('list')
...
async def choice_category(brand):
    with db.engine.connect() as conn:
        data = pd.read_sql_query(select(BrandsData.category).where(BrandsData.brand == brand).distinct(), conn).to_dict('list')
